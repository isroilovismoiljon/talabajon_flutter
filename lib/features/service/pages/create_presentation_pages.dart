                    ...
                    itemBuilder: (context, index) {
                      if (index == photoList.length) {
                        return GestureDetector(
                          onTap: () async {
                            if (photoList.length < int.parse(pageCountController.text) ~/ 3) {
                              final tanlanganRasmlar = await showModalBottomSheet<List<String>>(
                                context: context,
                                ...
                                builder: (_) => ShowSelectPhotosBottomSheet(
                                  pageCount: int.parse(pageCountController.text) ~/ 3 - photoList.length,
                                ),
                              );

                              if (tanlanganRasmlar != null && tanlanganRasmlar.isNotEmpty) {
                                setState(() {
                                  photoList.addAll(tanlanganRasmlar);
                                });
                              }
                            } else {
                              ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(
                                  content: Text("Siz yetarlicha resim tanlagansiz avval resimlarni o'chiring"),
